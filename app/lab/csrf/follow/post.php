<?php
    ...
    if( isset($_POST['chat-input']) ){
        ...
        $stmt = $db->prepare('INSERT INTO csrf_chat (authority, message) VALUES (:authority, :message)');
        $stmt->execute([':authority' => 'user', ':message' => $message]);
    
        $message_reply = $strings['message_reply'];
        $stmt_reply = $db->prepare('INSERT INTO csrf_chat (authority, message) VALUES (:authority, :message)');
        $stmt_reply->execute([':authority' => 'admin', ':message' => $message_reply]);
